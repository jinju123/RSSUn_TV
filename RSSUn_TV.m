function [U,cs,obj] = RSSUn_TV(M,Y,varargin)
...
%% -------------------------------------------------------------------------
%
% Copyright (January, 2011):        Jos?Bioucas-Dias (bioucas@lx.it.pt)
%
...
while (i <= AL_iters) && (sum(abs(res)) > tol1)
    ...
    for j=1:(n_reg+1)
        ...
        if  reg(j) == 1
          ...
          obj(i)=(1/2)*norm(Y-V{j},'fro')+(mu/2)*norm(M*U-V{j}-D{j},'fro')-(mu/2)*norm(D{j},'fro');
        end
        %  positivity   (V2)
        if  reg(j) == 2
            V{j} = max(U-D{j},0);
             obj(i)=obj(i)+(mu/2)*norm(U-V{j}-D{j},'fro')-(mu/2)*norm(D{j},'fro');
        end
        ...
        % l1 norm  (V4)
        if  reg(j) == 4
            V{j} = vector_hard_row(U-D{j},2*lambda_l1/mu);
            obj(i)=obj(i)+(mu/2)*norm(U-V{j}-D{j},'fro')-(mu/2)*norm(D{j},'fro');
        end
        % TV  (V5 and V6)
        if  reg(j) == 5
            ...
            obj(i)=obj(i)+lambda_TV*sum(sum(abs(V{j})));
        end
...
%          U_meanold= U_mean;
%          U_mean=(i/i+1)*U_meanold+(1/i+1)*U;
%         sto=norm(U-U_front,'fro')/(norm(Y,'fro'));
%         if sto<1e-10   
%         sto=1;
%         else
%             sto=sto;
%         end
      %  meansto(i)=norm(U_mean-U_meanold,'fro')/(norm(Y,'fro'));
%         F=sqrt(sum(U.^2,2));
%         ss=0;
%         for jjj=1:size(F)
%            if F(jjj)==0
...
%         end
%         
       % objfun(i)=(1/2)*norm(M*U-Y,'fro')+lambda_l1*l20norm(U)-(1/2)*norm(M*XT-Y,'fro')-lambda_l1*l20norm(XT);
%         if i==1
%             newsto=1;
%         else
%         newsto=abs((norm(U-XT,'fro')-norm(U_front-XT,'fro')))/norm(Y,'fro');
%         end
%         lres(i)=sum(abs(res));
...
%         end
   for j=1:(n_reg+1)
    ...
    cs(i)=sum(abs(ress))-2*sum(abs(ress));
    
    
    % compute RMSE
   if true_x
      %cs(i)= mu*norm(U-V{2},'fro')+mu*norm(U-V{5},'fro');
       % res_d(i)=mu*norm(U_mean-XT,'fro');
        sre(i)=20*log10(norm(XT,'fro')/norm(U-XT,'fro'));
%        xres(i)=norm(U-XT,'fro')/(norm(XT,'fro'));
%         meanxres(i)=norm(U_mean-XT,'fro')/(norm(XT,'fro'));
       %20*log10(norm(XT,'fro')/norm(U-XT,'fro'));%norm(Y-M*U,'fro'); %res=norm(Uj-XT,'fro')/(norm(XT,'fro'));%20*log10(norm(XT,'fro')/norm(Uj-XT,'fro'));  %norm(Uj-XT,'fro');
%         if  strcmp(verbose,'yes')
