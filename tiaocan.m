...
% [X_hat_cls] =  sunsal(A,Y,'lambda',0,'ADDONE','no','POSITIVITY','yes', ...
...
for i=1:6
  for j=1:6
   l=[1e-3 5e-3 1e-2 5e-2 1e-1 5e-1];
    t=[1e-4 5e-4 1e-3 5e-3 1e-2 5e-2];
lambda  = l(i);
lambda_TV =t(j);
[X_hat_tv_ni] = RSSUn_TV(A,Y,'MU',0.05,'POSITIVITY','yes','ADDONE','no', ...
                               'LAMBDA_1',lambda,'LAMBDA_TV', lambda_TV, 'TV_TYPE','niso',...
                               'IM_SIZE',[75,75],'AL_ITERS',1000, 'TRUE_X', X_true,  'VERBOSE','yes');
                          
SRE_RSStv_ni(i,j)= 20*log10(norm(X_true,'fro')/norm(X_hat_tv_ni-X_true,'fro'));   
  end
end
